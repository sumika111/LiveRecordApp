...
| avatar_url | text | | アバター画像URL（任意。設定で写真から選択時は Storage バケット `avatars` に保存→URLを保存。docs/STORAGE_AVATARS.md） |
| bio | text | | 一言・自己紹介（任意。migration_profiles_bio.sql で追加） |
| favorite_artists | text | | 好きなアーティスト（#アーティスト名 をスペース区切り。migration_profiles_favorite_artists.sql で追加） |
...
| memo | text | | その日の楽しかったこと（任意）。`migration_attendances_memo_photo.sql` |
| photo_url | text | | 思い出の写真1枚のURL（Supabase Storage）。同上 |
...
### 6. `follows`（フォロー関係）※友達・タイムライン用

| カラム | 型 | 制約 | 説明 |
|--------|------|------|------|
| id | uuid | PK, default gen_random_uuid() | |
| follower_id | uuid | NOT NULL, FK → auth.users(id) | フォローする側（自分） |
| following_id | uuid | NOT NULL, FK → auth.users(id) | フォローされる側（友達） |
| created_at | timestamptz | NOT NULL, default now() | |

- **一意** … (follower_id, following_id) で UNIQUE。自分自身はフォロー不可（CHECK）。
- **作成** … `docs/migration_follows_likes.sql` を実行。
...
|--------|------|------|------|
| id | uuid | PK, default gen_random_uuid() | |
| user_id | uuid | NOT NULL, FK → auth.users(id) | いいねしたユーザー |
| attendance_id | uuid | NOT NULL, FK → attendances(id) | 対象の記録 |
| created_at | timestamptz | NOT NULL, default now() | |

- **一意** … (user_id, attendance_id) で UNIQUE。1ユーザーが1記録に1いいねまで。
- **作成** … `docs/migration_follows_likes.sql` を実行。
