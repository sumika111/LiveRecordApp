...
| 3.2 | **どこから来たかで戻る**：詳細画面の戻るリンクを `from` で出し分け（TL・マイ・一覧・通知・管理者・編集・ホーム・友達） | ✅ | searchParams.from、各リンクに ?from= 付与 |
...
| # | タスク | 状態 | 備考 |
|---|--------|------|------|
| 5.1 | 未読件数は**メニューを開いたときだけ**取得（初回表示時はバッジなし） | ✅ | AppLayout：mount 時の fetch を削除 |
| 5.2 | 通知画面で **list 1本＋read 1本** に（count を list に含める） | ✅ | list API に read_at, likeCount, commentCount を追加、NotificationsContent は list のみ呼ぶ |
